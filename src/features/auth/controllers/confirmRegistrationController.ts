...
export const confirmRegistrationController = async (req: Request<{}, {}, {}, ICode>, res: Response<any>) => {
  ...
  console.log(user)
  if (user) {
    const updated = await userCollection.findOneAndUpdate({'emailConfirmation.confirmationCode': req.query.code}, {$set:{'emailConfirmation.isConfirmed': true}}, { returnDocument: 'after'})
